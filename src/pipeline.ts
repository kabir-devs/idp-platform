...
import { Deployment, Language, PipelineStep } from "./types";

const sleep = (ms: number) => new Promise((res) => setTimeout(res, ms));

const slugify = (repo: string) =>
  repo
    .toLowerCase()
    .replace(/^https?:\/\//, "")
    .replace(/^github\.com\//, "")
...
export function newDeploymentSkeleton(
  id: string,
  repo: string,
  language: Language,
...
function emitStep(deploymentId: string, step: PipelineStep) {
  store.events.emit(deploymentId, {
    type: "step-update",
    deploymentId,
    step,
  });
...
function emitDeployment(deployment: Deployment) {
  store.events.emit(deployment.id, {
